...
DEFAULT_CHECKPOINT_DIR = "master_thesis/classification_models/checkpoints/gat"
...
class GAT(torch.nn.Module):
    def __init__(
        ...
        self.convIn = GATConv(num_node_features, hidden_channels, heads=self.heads, dropout=dropout)
        self.convHidden = GATConv(hidden_channels * self.heads, hidden_channels, heads=self.heads, dropout=dropout)
        self.convFinal = GATConv(hidden_channels * self.heads, hidden_channels, heads=1, dropout=dropout)
...
class GATModel(BaseModel):
    ...
    def fit(self, X: Union[np.ndarray, List[Graph]], y: np.ndarray, dataset_config: Optional[Dict] = None):
        ...
        self.model = GAT(X_train[0].x.shape[1], 2, self.hidden_channels, self.heads, self.dropout).to(self.device)
